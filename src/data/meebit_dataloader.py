...
class MeebitDataset(torch.utils.data.Dataset):
    def __init__(
            ...
            if row["metadata"] is not None:
                ...
                if attributes is not None:
                    ...
                    image_filepath = os.path.join(
                        self.image_directory, f"{token_id}.{self.image_file_type}"
                    )
                    processed_attributes = {}
                    for attribute in attributes:
                        processed_attributes[attribute["trait_type"]] = attribute[
                            "value"
                        ]
        ...
        for trait_type in trait_pairs:
            ...
            label_encoder_mappings[trait_type] = label_encoder.fit(
                trait_pairs[trait_type]
            )
    ...
    def _encode_attributes(self, attributes):
        ...
        assert encoded_attributes.shape == torch.Size(
            [21]
        ), f"encoded_attributes.shape: {encoded_attributes.shape}"
...
class MeebitDataLoader(pl.LightningDataModule):
    ...
    def setup(self):
        dataset = MeebitDataset(
            self.image_directory, self.metadata, self.image_file_type
        )
        ...
        self.train, self.test, self.val = torch.utils.data.random_split(
            dataset, (train_samples, test_samples, val_samples)
        )
    ...
    def val_dataloader(self):
        return torch.utils.data.DataLoader(
            self.val, batch_size=self.batch_size, shuffle=False, drop_last=True
        )

    def test_dataloader(self):
        return torch.utils.data.DataLoader(
            self.test, batch_size=self.batch_size, shuffle=False, drop_last=True
        )
