...
@click.option(
    '--blockchain',
    default='eth',
    help='Blockchain to search over.'
)
def get_metadata(path_to_save_metadata: str, x_api_key: str, token_address: str, blockchain: str):
    """
    ...
    for idx in tqdm(range(offset, total+offset, page_size), total=total/page_size - 1):
        response = requests.get(
            f'{base_url}&offset={idx}&limit=500',
            headers=headers
        )
